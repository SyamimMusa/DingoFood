...
$sqlOrder = "SELECT * FROM orders WHERE customer_id = '$cusID';";
$result_order_list = mysqli_query($conn,$sqlOrder);
...
<head>
    ...
    <script src="../../js/requestRefund.js">
    </script>
...
date_default_timezone_set("Asia/Kuala_Lumpur");
 $time=date("h:i a");
?>
    <form class="refund" action="refundList.php" method="POST">
        <center>
...
//       $no=$no+1;
    ...
    $rows = $result_order_list->fetch_all(MYSQLI_ASSOC);
    $no = 0;
    foreach($rows as $row){
      $no++;
?>
                <tr>
                    ...
                    <td><a href="refundDetail.php?view_detail=<?php echo $row['order_id']; ?>">Details</td>
                    <td><input type="checkbox" name="choice[]" id="<?php echo $no?>"
                            value="<?php echo $row['order_detail']?>">

                    <td>
                        <div class="btn btn3" name="request_refund" data-id=<?php echo $row['order_id']; ?>
                            onClick="requestRefund(this);">Request
                            Refund</div>
                    </td>
...
?>
                    ...
                    <td></td>
                    <td>
                        <h3><input type="submit" class="btn btn3" id="btn" name="submit_add" value="SUM"></h3>
                    </td>
                </tr>
...
if(isset($_POST['isRefund'])){
    ...
    $refund_time = date("h:i a");

    $sql = "INSERT into refund (customer_id,order_id,item,price,refund_status,refund_time) VALUES ('$customer_id','$order_id', 
    '$item',' $total', 'Pending....','$refund_time' );";
...
?>
 ...
 ?><tr>
        ...
        ?>
                    <input type="submit" value="cancel request" class="btn btn1" name="Submit1"
